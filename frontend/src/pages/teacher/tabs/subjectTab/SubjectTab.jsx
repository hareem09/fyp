      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {subjects.length === 0 ? (
          ...
          subjects.map((subject, i) => (
            <div key={subject._id || i} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">

              {/* Subject Header */}
              <div className="flex items-start justify-between mb-4">
                <div className="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center">
                  <span className="text-purple-600 font-bold text-sm">
                    {subject.code ? subject.code.slice(0, 2) : '??'}
                  </span>
                </div>
              ...
              {/* Schedule */}
              {Array.isArray(subject.schedule) && subject.schedule.length > 0 && (
                <div>
                  ...
                  <div className="space-y-1">
                    {subject.schedule.map((s, j) => (
                      <div
                        key={j}
                        className="flex justify-between text-xs bg-blue-50 rounded-lg px-3 py-1.5"
                      >
